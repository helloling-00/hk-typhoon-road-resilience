"""
图67 横版：1 行 × 3 列  —  Sep 23 Signal-3 midday churn @ 13:00 (slot 26)
  Panel 1: Normal workday baseline
  Panel 2: Sep 23 actual speeds
  Panel 3: Binary faster (33.9%) / slower (35.8%) by road length
"""
...
bl_wk_slot = bl[(bl["day_type"]=="WORKDAY") & (bl["slot"]==SLOT_AM)][
...
def plot_speed(ax, spd_series, title, subtitle):
    ...
    _set_extent(ax)
    _add_basemap(ax)
    ax.set_title(title, color=TXT, fontsize=18, fontweight="bold", pad=10)
    ax.text(0.5, 0.01, subtitle, transform=ax.transAxes,
            ha="center", fontsize=12, color=SUB)
...
def plot_deviation(ax, dev_series, title, subtitle):
    ...
    if len(slower):
        slower.plot(ax=ax, color="#d62728", linewidth=1.0, alpha=0.90,
                    zorder=4, label=f"Slower (dev < −0.03)  —  35.8% by length")
    if len(faster):
        faster.plot(ax=ax, color="#2ca02c", linewidth=1.0, alpha=0.90,
                    zorder=5, label=f"Faster (dev > +0.03)  —  33.9% by length")
    _set_extent(ax)
    _add_basemap(ax)
    ax.set_title(title, color=TXT, fontsize=18, fontweight="bold", pad=10)
    ax.text(0.5, 0.01, subtitle, transform=ax.transAxes,
...
plot_speed(axes[0], bl_wk_slot,
           "Normal Workday Baseline  —  Midday (13:00)",
           "Mean of all non-typhoon workdays at slot 26")
plot_speed(axes[1], spd_event,
           "Ragasa  Sep 23  Signal 3  —  13:00",
           "Pre-escalation midday, day before Signal 8/10")
plot_deviation(axes[2], dev,
               "Pre-Event Midday Bidirectional Churn under Signal 3",
               "Ragasa, Sep 23, 13:00   ·   "
...
plt.setp(cb1.ax.xaxis.get_ticklabels(), color=TXT, fontsize=12)

fig.suptitle(
    "Pre-Event Midday Bidirectional Churn under Signal 3  (Ragasa, Sep 23, 13:00)",
    color=TXT, fontsize=22, fontweight="bold")

out = f"{OUT}/图67_preS3_midday_churn_3panel.png"
